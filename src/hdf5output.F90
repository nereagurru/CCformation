...
! Create, append and read hdf5 files needed for 2DMC
! --------------------------------------------------------------------------------
! File structure
! --------------------------------------------------------------------------------
! Attributes
...
module hdf5output
  ...
  contains
  subroutine hdf5_file_write(this, dir, swrm, time, resdt, mode, outnr, mswarm, Ntot &
#ifdef LOCAL_SIM
                              &, imax &
#endif
                              &)
    ...
    filepath = trim(dir)//"/"//trim(filename)
    write(*,*) filepath
    !call h5open_f(error)
    if (mode .eq. 'create') then
      call h5open_f(error)
      call hdf5_file_create(this, filepath, outnr , mswarm, resdt, Ntot &
#ifdef LOCAL_SIM
                           &, imax &
#endif
                           &)     ! creates file
  ...
  subroutine swarm_unit_conversion(swrm, tempswarm)
    ...
    do i=1,size(swrm)
      ...
      tempswarm(i)%f_CH = swrm(i)%f_CH
      tempswarm(i)%rigid = swrm(i)%rigid

      tempswarm(i)%rigid_mass = swrm(i)%rigid_mass
#ifdef LOCAL_SIM
      tempswarm(i)%tfor = swrm(i)%tfor/year
#endif
#ifdef GLOBAL
      tempswarm(i)%tfor = swrm(i)%tfor/year
#endif
      tempswarm(i)%plt = swrm(i)%plt
    end do
...
! subroutine to add the swarm object as a compound datatype to the HDF5 file
  subroutine create_swarm_type(this,tempswrm)
    ...
    call h5tinsert_f(this%memtype, "f_CH", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%f_CH)), &
                                                            H5T_NATIVE_DOUBLE, this%error)
    call h5tinsert_f(this%memtype, "rigid", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%rigid)), &
                                                      H5T_NATIVE_INTEGER, this%error)
    call h5tinsert_f(this%memtype, "rigid_mass", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%rigid_mass)), &
                                                            H5T_NATIVE_DOUBLE, this%error)
#ifdef LOCAL_SIM
   call h5tinsert_f(this%memtype, "tfor", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%tfor)), &
                     H5T_NATIVE_DOUBLE, this%error)
#endif
#ifdef GLOBAL
   call h5tinsert_f(this%memtype, "tfor", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%tfor)), &
                     H5T_NATIVE_DOUBLE, this%error)
#endif
    call h5tinsert_f(this%memtype, "plt", H5OFFSETOF(C_LOC(swrm(1)),C_LOC(swrm(1)%plt)), &
                                                            H5T_NATIVE_INTEGER, this%error)                                                          
    return
  end subroutine create_swarm_type
  
  ! subroutine to create a hdf5 file 
  subroutine hdf5_file_create(this, filename, nout, mswarm, restd, Ntot &
#ifdef LOCAL_SIM
                              &, imax &
#endif
                              &)
    ...
    call hdf5_write_att(this%id, 'code', '2DMC v0.1')
    call hdf5_write_att(this%id, 'author', 'Dr Joanna Drazkowska')
    call hdf5_write_att(this%id, 'output_number', nout)
    call hdf5_write_att(this%id, 'mass_of_swarm', mswarm)
    call hdf5_write_att(this%id, 'timestep', restd)
    call hdf5_write_att(this%id, 'Ntot', Ntot)
#ifdef LOCAL_SIM
   call hdf5_write_att(this%id, 'imax', imax)
#endif    
  ...
  subroutine hdf5_file_read(file, Ntot, swrm, nout, mswarm, time, resdt &
#ifdef LOCAL_SIM
                                 &, imax, feed &
#endif
                                 &)
    ...
    ! open file and group 
    call h5open_f(error)
    call h5fopen_f(file, H5F_ACC_RDONLY_F, this% id, this% error)
    call check_error(this% error, 'error opening the file restart.h5')
    call h5gopen_f(this% id, 'swarms', this%swarms, this% error)
    call check_error(this% error, 'swarms group opening error')
#ifdef LOCAL_SIM
   if (feed) then
      call hdf5_read_att(file, nout,mswarm, resdt, Ntot, imax, feed)
      imax = Ntot
   else
      call hdf5_read_att(file, nout,mswarm, resdt, Ntot, imax, feed)
   endif
   ...
   call hdf5_read_att(file, nout,mswarm, resdt, Ntot)

#endif

#ifdef LOCAL_SIM
    if (.not.allocated(swrm)) allocate( swrm(imax) )
#else
  ...
  subroutine hdf5_read_att(file, nout, mswarm, resdt, Ntot &
#ifdef LOCAL_SIM 
                           &, imax, feed &
#endif
                           &)
    ...
    integer(HID_T) :: att1, att2, att3, att4
#ifdef LOCAL_SIM 
    integer(HID_T) :: att5
#endif
    integer :: nout
   ...
   ! timestep
    CALL h5aopen_name_f(this%id, 'timestep', att2, this%error)
    call check_error(this%error, 'error opening the attribute timestep')

    f_ptr = C_LOC(resdt)
    CALL h5aread_f(att2, H5T_NATIVE_DOUBLE, resdt, dims, this%error)
    CALL h5aclose_f(att2, this%error)
 
    ! Ntot
    CALL h5aopen_name_f(this%id, 'Ntot', att3, this%error)
    call check_error(this%error, 'error opening the attribute Ntot')

    f_ptr = C_LOC(Ntot)
    CALL h5aread_f(att3, H5T_NATIVE_INTEGER, Ntot, dims, this%error)
    CALL h5aclose_f(att3, this%error)  

    ! nout
    CALL h5aopen_name_f(this%id, 'output_number', att4, this%error)
    call check_error(this%error, 'error opening the attribute output_number')

    f_ptr = C_LOC(outnr)
    CALL h5aread_f(att4, H5T_NATIVE_INTEGER, nout, dims, this%error)
    CALL h5aclose_f(att4, this%error)
    
#ifdef LOCAL_SIM
   ! imax
   if (feed .eqv. .False.) then
      CALL h5aopen_name_f(this%id, 'imax', att5, this%error)
      call check_error(this%error, 'error opening the attribute imax')
      
      f_ptr = C_LOC(imax)
      CALL h5aread_f(att5, H5T_NATIVE_INTEGER, imax, dims, this%error)
      CALL h5aclose_f(att5, this%error)
   endif
#endif
